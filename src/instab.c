...
#define INIT_INS(_ins, _addr, _code) ins_table[INS_INDEX(_ins)][ADDR_INDEX(_addr)] = insobj_new(_ins, _addr, _code)
#define BYTE NUM

// typedef struct insobj * p_insobj;

struct insobj * ins_table[INS_COUNT][INS_ADDR_COUNT];
...
void instab_init(){
	int m,n;
	for(m=0; m<INS_COUNT; ++m){
		for(n=0; n<INS_ADDR_COUNT; ++n){
			ins_table[m][n] = NULL;
		}
	}
	instab_initall();
...
struct insobj * insobj_new(t_token ins, t_token addr, t_value code){
	struct insobj * io = (struct insobj *)MALLOC(sizeof(struct insobj));
	io->ins = ins;
	io->addr = addr;
	io->code = code;
	return io;
...
struct insobj * instab_get(t_token ins, t_token addr){
	return ins_table[INS_INDEX(ins)][ADDR_INDEX(addr)];
}
